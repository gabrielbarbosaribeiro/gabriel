...
```{r}
vendas_totais_categoria <- dados_vendas_longo %>%
  group_by(categoria) %>%
  summarize(vendas_media = mean(valor_vendas)) %>%
  arrange(desc(vendas_media))

# visualiza o resultado
vendas_totais_categoria
```
...
```{r}
vendas_mobiliario <- dados_vendas_longo %>%
...
```{r}
# Calcula o total mensal por categoria
vendas_mensais_categoria <- dados_vendas_longo %>%
  group_by(categoria, mes) %>%
  summarize(vendas_mensais = sum(valor_vendas), .groups = "drop")

# Calcula os indicadores por categoria
desempenho_categoria <- vendas_mensais_categoria %>%
  group_by(categoria) %>%
  summarize(
    total_vendas = sum(vendas_mensais),
    media_mensal = mean(vendas_mensais),
    minimo_mensal = min(vendas_mensais),
    maximo_mensal = max(vendas_mensais)
  ) %>%
