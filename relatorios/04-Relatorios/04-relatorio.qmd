...
```
```{r}
# exibe as ultimas 6 linhas do objeto gapminder
tail(gapminder, 10)
```
...
```{r}
# seu código aqui
variaveis_selecionadas <- select(gapminder, -gdpPercap, -pop)
...
```{r}
# seu codigo aqui
paises_asia_condicoes <- gapminder %>%
  filter(
    continent == "Asia",
    pop > 50000000,
    year == 2007
...
```{r}
dados_novavariavel <- gapminder %>%
  mutate(pop_em_milhoes = pop / 1000000)
head(dados_novavariavel)
```
...
```{r}
dados_economia_grande <- gapminder %>%
  mutate(economia_grande = ifelse(pop > 50000000, "Sim", "Não"))

print(dados_economia_grande, n = Inf)
```
...
```{r}
multiplas_categorias <- gapminder %>%
  filter(year == 2007) %>%
  mutate(categoria_expectativa = case_when(
    lifeExp < 60 ~ "Baixa",
...
print(expectativa_por_continente)
```
...
pop_total_ano2007 <- gapminder %>%
  ...
  arrange(continent, pop_total)
  
print(pop_total_ano2007)
```
...
```{r}
tabela_continent <- gapminder %>% 
  group_by(continent) %>% 
  summarize(
    numero_paises = n(),
    pib_per_capita_medio = mean(gdpPercap),
    melhor_filial_pib = max(gdpPercap)
  ) %>%
  arrange(continent)
...
```{r}
# seu codigo aqui
evolucao_exp_vida_americas <- gapminder %>%
  filter(continent == "Americas") %>%
  group_by(year) %>%
  summarize(
    expectativa_media_americas = mean(lifeExp),
    num_paises_analisados = n()
  ) %>%
  arrange(year)

evolucao_exp_vida_americas
```
...
ranking_paises <- gapminder %>%
  filter(year == 2007) %>% # Filtra dados apenas de 2007
  select(country, gdpPercap) %>% # Seleciona colunas relevantes
  arrange(gdpPercap) %>% # Ordena do menor para o maior PIB
  head(5) # Mostra os 10 primeiros resultados
...
```{r}
# seu codigo aqui
paises_americas_pop_desc <- gapminder %>%
  filter(continent == "Americas", year == 2007) %>%
  select(country, pop) %>%
  arrange(desc(pop))

paises_americas_pop_desc
```
...
```{r}
# seu codigo aqui
ranking_continentes_expectativa <- gapminder %>%
  filter(year == 2007) %>%
  group_by(continent) %>%
  summarize(
    expectativa_media_continente = mean(lifeExp),
    num_paises = n()
  ) %>%
  arrange(desc(expectativa_media_continente))

ranking_continentes_expectativa
```
